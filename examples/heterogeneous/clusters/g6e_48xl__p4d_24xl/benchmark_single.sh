#!/usr/bin/env bash
# 2-node benchmark single run (g6e.48xl + p4d.24xl, PP=4 TP=4).
#
# Usage:
...
sed -i 's/^  train_steps: .*/  train_steps: 2/' "$CONFIG"
...
cat > "$OUT_DIR/meta.json" <<EOF
{
  ...
  "model": "$MODEL",
  "cluster": "$CLUSTER",
  "gpu_node0": "L40S ×8 (g6e.48xlarge, stages 0+1)",
  "gpu_node1": "A100 ×8 (p4d.24xlarge, stages 2+3)",
